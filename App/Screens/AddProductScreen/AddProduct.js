...
const ProductPage = (props) => {
  ...
  useEffect(() => {
    load();
    if(global.product && global.product._id){
    setProduct(global.product)
    props.navigation.setOptions({headerTitle: 'Edit Product'})
  }
  }, [isFocused, setCat]);
  ...
  const onSubmit = e => {
    e.preventDefault();
    console.log('prodeict details================',product);
    ...
    if(product._id){
      ...
      .catch(err => {
        ...
        alert(err.response.data.message||'Error while updating product')
      });
            ...
            return (
              <View
                ...
                <TextInput
                  ...
                  name="Product Unit"
                  onChangeText={e =>
                    dynamicChangeHandler('offerPrice', index, 'unit', e)
                  }
                />
